...
"""
    ...
    try:
        ...
        logger.exception("Planner model call failed: %s", exc)
        ...
        analytics_service.record_request(
            repository=state.get("repo_url") or state.get("repo_path") or os.getenv("REPO_NAME", "unknown"),
            user_prompt=user_prompt,
            selected_files=[block.get("file") for block in retrieved_blocks[:5] if block.get("file")],
            total_files=len(state.get("files", [])) or len(retrieved_blocks),
