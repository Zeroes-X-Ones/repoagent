...
# NOTE: The default OpenRouter endpoint now prefers the local Paritok proxy so
# requests are routed through the configured gateway unless an explicit override is supplied.
DEFAULT_OPENROUTER_API_URL = "http://127.0.0.1:8080/v1/chat/completions"


def _resolve_openrouter_api_url() -> str:
    """Resolve the OpenRouter endpoint while supporting proxy-style environment overrides."""
    # Prefer an explicit OpenRouter URL when one is provided.
    explicit_url = os.getenv("OPENROUTER_API_URL", "").strip()
    if explicit_url:
        return explicit_url

    # Support both OPENROUTER_BASE_URL and OPENAI_BASE_URL for local proxies such as Paritok.
    base_url = (
    ...
    # If the environment already points at a complete chat-completions endpoint, use it as-is.
    if "chat/completions" in base_url:
        return base_url

    # For local proxies such as Paritok, the typical OpenAI-compatible path is /v1/chat/completions.
    # Preserve the previous OpenRouter path shape when the base is already an OpenRouter-style API root.
    if "/v1" in base_url or "/api/v1" in base_url:
        return f"{base_url.rstrip('/')}/chat/completions"

    return f"{base_url.rstrip('/')}/v1/chat/completions"


OPENROUTER_API_URL = _resolve_openrouter_api_url()
...
def _log_provider_request(provider: str, model: str, url: str) -> None:
    """Log which provider and transport handled the outbound request."""
    # NOTE: OpenRouter requests now use the proxy-aware endpoint when available,
    # but still preserve the previous direct OpenRouter behavior by default.
    if provider == "openrouter":
        transport = "Paritok" if "openrouter.ai" not in url else "OpenRouter"
    else:
        transport = provider.title()

    logger.info(
        "LLM request: provider=%s, transport=%s, model=%s, url=%s",
        provider,
        transport,
        model,
        url,
...
def _bedrock_chat(system_prompt, user_prompt, model):
    # NOTE: Bedrock support remains unchanged; this block only adds request logging.
    _log_provider_request("bedrock", model, BEDROCK_OPENAI_API_URL)
...
def _openrouter_chat_with_key(system_prompt, user_prompt, model, api_key):
    # NOTE: OpenRouter requests now resolve through proxy-aware configuration.
    resolved_url = _resolve_openrouter_api_url()
    _log_provider_request("openrouter", model, resolved_url)
...
def _groq_chat(system_prompt, user_prompt, model):
    # NOTE: Groq support remains unchanged; this block only adds request logging.
    _log_provider_request("groq", model, "groq")
    try:
